...
class Identify {
  ...
  */
  ...
  KeyDefine GetToken() { 
    ...
    while ( bool_true ) {
      ...
      if ( gGetChar != '\0' ) {      // last round
        if ( MODE ) {
          cout << "[debug] GetToken() : deal multi token! >>" << gGetChar << "<<" << endl ;
        } // if
        ...
        else if ( getString == "\'" || getString == "(" || getString == ")" ) {
          last_round = false ;
          bool_true = false ;
          store_line = gLine ;
          store_column = gColumn ;
      ...
      if ( ! bool_true ) 
        ...
        if ( exception_semi ) {    
          ...
          if ( chartoString == "\n" ) {
            ...
            + Int_To_String( gLine ) + " Column " + Int_To_String( gColumn ) << endl << endl ;
  ...
  */
  ...
  void DetectExit( Tree * head ) {
    if ( head != NULL ) {
      if ( head->m_left != NULL ) {
        if ( head->m_left->m_key.GetType() == SYMBOL && head->m_left->m_key.GetKey() == "exit" ) {
...
class Expression {
  ...
  void SeparateTokenchain( vector<KeyDefine> tokenchain, vector<KeyDefine> & left_tokenchain,
    ...
    if ( MODE ) {
      ...
      if ( new_extend ) 
        cout << "New Extend = true " << endl ;
      else 
        cout << "New Extend = false " << endl ;
      cout << endl ;
    } // if
    ...
    if ( new_extend ) {
      ...
      if ( tokenchain.front().GetType() == LEFT_PAREN && tokenchain.back().GetType() == RIGHT_PAREN ) { 
        // 切最大括號
        for ( int i = 0, count_paren = 0 ; i < tokenchain.size() ; i++ ) {
  ...
  Tree * BuildTree_Interfaces( vector<KeyDefine> reserve ) {
    ...
    if ( tokenchain.size() == 1 ) {
      ...
      head = BuildTree( tokenchain, false ) ;  // NEW_INTERNAL
