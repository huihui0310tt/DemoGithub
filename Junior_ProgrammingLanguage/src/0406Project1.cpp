...
class Identify {
  ...
  */
  ...
  KeyDefine GetToken() { 
    ...
    if ( MODE ) {
      cout << "   [debug] GetToken()  " << "[" << temp.GetLine() << ":" << temp.GetColumn() << "]"
                                        << temp.GetKey() << " "  ;
      temp.PrintType( true ) ;      
    } // if
  ...
  */
  ...
  void DetectAtoM() {
    ...
    if ( MODE ) {
      cout << "   [debug] in DetectAtoM Front()  >>" ;
      for ( int i = 0 ; i < gDealToken.size() ; i++ ) {
        cout << gDealToken[i].GetKey() << " " ;
      } // for
...
class Expression {
  ...
  void Print_SingleQuote( Tree * head, int count ) {
  ...
  void Print( Tree * head, int count ) {
    ...
    while ( bool_true ) {
      ...
      if ( head->m_left->m_key.GetType() == INTERNAL_NODE ) {
      ...
      else if ( head->m_left->m_key.GetType() == SINGLE_QUOTE ) {
        if ( ! first_round ) {
          cout << "( " ;
        } // if
        ...
        if ( first_round ) 
          Print_SingleQuote( head->m_right->m_left, count-1 ) ;
        else
          Print_SingleQuote( head->m_right->m_left, count ) ;
  ...
  vector< vector<KeyDefine> > GetItem( vector<KeyDefine> tokenchain ) {
    vector< vector<KeyDefine> > item ;
    vector<KeyDefine> item_temp ;
    ...
    while ( count <= 2 ) {
      for ( int i = 0 ; i < tokenchain.size() ; i++ ) {   
        if ( tokenchain[i].GetType() == LEFT_PAREN ) 
          count_paren++ ;
        else if ( tokenchain[i].GetType() == RIGHT_PAREN ) 
          count_paren-- ;

        item_temp.push_back( tokenchain[i] ) ;
    ...
    if ( MODE ) {
      DEBUG_Print_tokenchain( tokenchain ) ;
  ...
  Tree * BuildTree_Right( vector< vector<KeyDefine> > item ) {
    // 只遞迴Internal Node   

    if ( MODE ) {
      cout << "BuildTree_Right() trace" << endl << "  " ;
      DEBUG_Print_ITEM( item ) ;
    } // if
    ...
    item_temp = item.front() ;
    item.erase( item.begin() ) ;

    if ( item_temp.size() == 1 ) {
      head->m_left = new Tree() ;
      head->m_left->m_key = item_temp.front() ;
      if ( MODE ) {
        cout << "put " << item_temp.front().GetKey() << endl ;
      } // if
    } // if
    else 
      head->m_left = BuildTree_Left( item_temp ) ;
 


    if ( ! item.empty() ) {
      item_temp = item.front() ;
      if ( item_temp.front().GetType() == DOT ) {
  
        item.erase( item.begin() ) ;
        item_temp = item.front() ;
        if ( item_temp.size() == 1 ) {
          head->m_right = new Tree() ;
          head->m_right->m_key = item_temp.front() ;
        } // if
        else {
          if ( item_temp.front().GetType() == LEFT_PAREN ) {
            item_temp.erase( item_temp.begin() ) ;
            item_temp.erase( item_temp.end() ) ;
            head->m_right = BuildTree_Right( GetItem( item_temp ) ) ;
          } // if
          else 
            head->m_right = BuildTree_Right( item ) ;
        } // else
      } // if
      else {
        head->m_right = BuildTree_Right( item ) ;
  ...
  Tree * BuildTree_Left( vector <KeyDefine> tokenchain ) {
    // 只遞迴Internal Node   
    if ( MODE ) {
      cout << "[debug] BuildTree_Left() "  << endl ;
      cout << "Print tokenchain" << endl ;
    ...
    Tree * head = BuildTree_Right( GetItem( tokenchain ) ) ;   
  ...
  void BuildTree_Interfaces() {
    ...
    vector<KeyDefine> tokenchain ;  // gdealtoken convert
    KeyDefine temp ;
    ...
    for ( int i = 0 ; i < gDealToken.size() ; i++ ) {             // '('  ')' -> "()"  
      ...
      if ( gDealToken.at( i ).GetType() == LEFT_PAREN ) {
        ...
        temp = gDealToken.at( i ) ;
        have_left = true ;
      } // if
      else {
        if ( have_left ) {
          if ( gDealToken.at( i ).GetType() == RIGHT_PAREN ) {
            temp.SetType( NIL ) ;
            temp.SetKey( "nil" )  ;
            tokenchain.push_back( temp ) ;  
          } // if
            ...
            tokenchain.push_back( temp ) ;
            tokenchain.push_back( gDealToken.at( i ) ) ;
          } // else
    ...
    if ( tokenchain.size() == 1 ) {
      ...
      g_Tree = BuildTree_Right( GetItem( tokenchain ) ) ; 
    } // else
  ...
  void PrintSExp() {
    if ( MODE ) {
      cout << "[info] PrintSExp() " << endl ;
      ViolatePrint( g_Tree, 1 ) ;
      cout << "-------" << endl ;
    } // if
