...
class Evaluate_Exp {
 // error : throw 4
  ...
  Tree * GetBinding( Tree * head, bool getAddress ) {
    ...
    if ( head->m_key.GetType() == INTERNAL_NODE ) 
      ...
      if ( head->m_key.GetType() == SYMBOL && ! Is_Keywords( head->m_key.GetKey() ) ) {
        Tree * binding = GetDefine( head->m_key.GetKey(), g_Define_Binding, getAddress ) ;
        if ( binding == NULL ) {
  ...
  bool CompareKey( string type, string key1, string key2 ) {

    if  ( type == ">" ) {
      if ( String_to_Int( key1 ) > String_to_Int( key2 ) ) 
        return true ;
      else 
        return false ;
    } // if
    ...
    try {
      ...
      if ( type == "cons" ) {  // (cons a . b)
        ...
        if ( have_dot ) {
          ...
          // else 
          //   evaluate->m_left = GetBinding( head->m_right->m_left, false ) ;
          
          // if ( head->m_right->m_right->m_left->m_key.GetType() == INTERNAL_NODE ) 
          //   evaluate->m_right = Eval_SExp( head->m_right->m_right->m_left ) ;
          // else 
          //   evaluate->m_right = GetBinding( head->m_right->m_right->m_left, false ) ;
            ...
            if ( head->m_left != NULL ) {
              if ( head->m_left->m_key.GetType() != INTERNAL_NODE ) {
                if ( walk == NULL ) {
                  ...
                  evaluate->m_left = GetBinding( head->m_left, false ) ;
                  walk = evaluate ;
                } // if
                else {
                  ...
                  walk = walk->m_right ;
                  walk->m_left = GetBinding( head->m_left, false ) ;
                } // else
              } // if
              else {  //  need to care
                if ( walk == NULL ) {
  ...
  Tree * Evaluate_Part_Accessors( Tree * head, string type ) {
    ...
    try {
      ...
      else if ( head->m_right->m_left->m_key.GetType() != INTERNAL_NODE ) {
          ...
          while ( head != NULL ) {
            if ( type == "string-append" ) {
              ...
              if ( compare1 == "" ) 
                ...
                if ( ! CompareKey( type, compare1, head->m_left->m_key.GetKey() ) ) {
                  evaluate->m_key = f ;
                }  // if

                // if ( type == ">" || type == "string>?" ) {
                //   if ( String_to_Int( compare1 ) <= String_to_Int( head->m_left->m_key.GetKey() ) ) 
                //     evaluate->m_key = f ;
                // } // if
                // else if ( type == ">=" ) {
  ...
  Tree * GetDefine( string name, vector<Tree *> g_Define_Binding, bool getAddress ) {
    for ( int i = 0 ; i < g_Define_Binding.size() ; i++ ) {
      if ( g_Define_Binding.at( i )->m_left->m_key.GetKey() == name ) {
  ...
  Tree * Eval_SExp( Tree * tree ) {
    ...
    string operate_type = tree->m_left->m_key.GetKey() ;  // 展開
    if ( GetDefine( operate_type, g_Define_Binding, false ) != NULL ) {   // Define change
    ...
    try {
      ...
      if ( operate_type == "cons" || operate_type == "list" ) {
        ...
        result = GetDefine( tree->m_left->m_key.GetKey(), g_Define_Binding, false ) ;
...
int main() {
  ...
  while ( bool_true ) {
    try {
      ...
      // cout << "---debug for define" << endl ;
      // for ( int i = 0 ; i < g_Define_Binding.size() ; i++ ) {
      //   print_exp.PrintSExp( g_Define_Binding[i] ) ;
      // } // for
      // cout << endl << endl << endl ;
      ...
      if ( num == 0 ) {    // (exit)
        ...
        if ( gDealToken[0].GetType() == SYMBOL ) {
          if ( Is_Keywords( gDealToken[0].GetKey() ) ) {
            ...
            Tree * get_atom_binding = evaluate_exp.GetDefine( gDealToken[0].GetKey(), 
                                                              g_Define_Binding, false ) ;
